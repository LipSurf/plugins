	...
	<div>
		...
		<Slide ref="slide1" timing="1">
			...
			<div class="perms" ref="perms">
				<div rel="mic-perm" style="display: inline-block;" class="notice" :class="{success: parent.hasMicPerm, failure: parent.hasMicPerm === false}">
					<i class="icon" :class="{'check-circle': $parent.hasMicPerm, error: !$parent.hasMicPerm}"></i>&nbsp;
					<span>{{ $parent.hasMicPerm ? 'Has microphone permission.' : 'Needs microphone permission.' }}</span>
				</div>
		...
		<Slide ref="slide2" timing="4">
			...
			<ul>
				<li>
					...
					<a href="#/slide/2" @click="$parent.openFlags">in your flags</a> if you're using Chrome 66 or newer.
					<ul>
