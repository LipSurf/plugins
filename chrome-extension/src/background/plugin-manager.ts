...
export class PluginManager extends StoreSynced {
    ...
    async injectCmdCodeIntoPage(tabId: number, url: string): Promise<void> {
		...
		return new Promise<void>((resolve, reject) => {
			chrome.tabs.executeScript(tabId, {code: compiledCsStr, runAt: "document_start"}, function() {
                // executeScript won't work on chrome-extension://* pages
				if (chrome.runtime.lastError) {
					chrome.tabs.sendMessage(tabId, {code: compiledCsStr}, function() {
                        resolve();
                    });
				} else {
					resolve();
