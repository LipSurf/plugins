...
function scroll(direction, little = !1) {
  ...
  if (direction === "hd" || direction === "hu") {
    ...
    let codes;
    return direction === "t" || direction === "b" ? codes = [key] : codes = new Array(14 * littleFactor).fill(key), chrome.runtime.sendMessage({
      type: "pressKeys",
      payload: {codes, nonChar: !0}
    }), PluginBase.util.sleep(100);
  } else {
...
var Scroll_default = {
  ...
  contexts: {
    "Auto Scroll": {
      commands: ["Speed Up", "Slow Down", "Stop"]
    }
  },
  destroy() {
    stopAutoscroll();
