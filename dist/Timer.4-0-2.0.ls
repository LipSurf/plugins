// dist/tmp/Timer/Timer.js
var SET_TIMER_REGX = /\bset (?:(.*) )?timer (?:for )?(\d+) (seconds|minutes?|hours?)(?:(?: and)? (?:(?:(\d+) (seconds|minutes?))|(?:(?:a (?:(half)|(quarter))))))?\b/, PARTIAL_SET_TIMER_REGX = /\bset\b(.* )?(timer)?\b/, Timer_default = { ...PluginBase, languages: {}, niceName: "Timer", description: "Tools for setting timers.", version: "4.0.2", apiVersion: 2, match: /.*/, authors: "Miko", commands: [{ name: "Set Timer", description: 'Shows a notification and speaks "timer elapsed" (audio) after the specified duration.', global: !0, match: { description: "set [timer name (optional)] timer for [n] [seconds/minutes/hours]", fn: ({ preTs, normTs }) => {
  ...
  !isNaN(seconds2) && seconds2 && (unit2.startsWith("minute") && (seconds2 *= 60), seconds += seconds2), half ? unit.startsWith("minute") ? seconds += 30 : seconds += 1800 : quarter && (unit.startsWith("minute") ? seconds += 15 : seconds += 900), console.log(`total seconds ${seconds}`), setTimeout(() => {
    ...
    title = title[0].toUpperCase() + title.slice(1, title.length), chrome.notifications.create({ type: "basic", title, message: `"${normTs}"`, iconUrl: "assets/icon-timer-48.png", requireInteraction: !0 }), chrome.tts.speak(title);
  }, seconds * 1e3);
} }] };
Timer_default.languages.ru = { niceName: "Таймер", description: "Поиск в Google", authors: "Hanna", commands: { "Set Timer": { name: "Установить таймер", description: 'Показывает уведомление и говорит "Таймер установлен" после того, как назван промежуток времени.', match: { description: 'Скажите "Установить таймер [имя таймера (не обязательно)] на x секунд/минут/часов"', fn: ({ preTs, normTs }) => {
...
allPlugins.Timer = (() => {
  ...
  return { ...PluginBase, commands: { "Set Timer": { match: { en: ({ preTs, normTs }) => {
    let match = normTs.match(SET_TIMER_REGX);
    if (match) {
      let endPos = match.index + match[0].length;
      return [match.index, endPos, match];
